...
src/
  ...
  data/
    ...
    docsManifest.js     AUTO-GENERATED by scripts/build.js — gitignored. Reference Library is
                         GitHub-Release-hosted now (docs/adr/0039), not purely LFS: each entry's
                         `src` resolves to a local path if the real PDF is on disk, else a Release
                         asset URL (see referenceLibraryManifest.js/releaseConfig.js below)
    referenceLibraryManifest.js  COMMITTED (not gitignored) catalog every known Reference Library
                         PDF is generated from — the source docsManifest.js's build-time scan reads
    releaseConfig.js    releaseAssetUrl() — builds a GitHub Release download URL (ADR 0039)
  domain/          pure, tested, DOM-free business logic
    ...
    copilot.js     advise(campaign) -> observation/consequence/opportunity/suggestion; also
                    gatherSceneContext()/buildStoryOptions() — cumulative WHO/WHERE/WHY-aware
                    suggestions (ADR 0039), the direct precedent Phase 12's Dashboard extends
    foreshadowing.js  a GM's own plant/pay-off to-do list (campaign.foreshadowing[])
    worldFlags.js  lightweight known/unknown/confirmed/false fact ledger (campaign.worldFlags[])
...
- Playwright browser tests (when used) launch Chromium with
  ...
  **Confirmed genuinely absent** in this environment as of 2026-07-15 (no
  `chromium-cli`, no Playwright) — don't assume it'll appear; a
  `jsdom`-based smoke test driving the actual built bundle is a workable,
  lighter-weight substitute (see the Phase 5 verification approach in git
  history). For verifying an *external* resource specifically (a deployed
  URL, a GitHub Release asset) rather than the app's own UI, direct
  `curl`/the GitHub REST API worked well without any browser at all — used
  successfully for the Reference Library Release migration (ADR 0039);
  prefer that over reaching for browser automation when there's no DOM to
  actually render.
...
don't re-derive either from git log unless neither is current. Don't
restate phase status here when updating this file — update `PROGRESS.md`'s
Status Summary instead, so there's exactly one place this drifts from
reality.
...
- **No two docs get to disagree about current reality.** Design principles
  ...
  older ADR's Status line, the way `0003` points at `0004`, or `0040`
  recording its own deliberate exception to Article X), or a design doc's
  prose vs. what the code actually does now (the code wins; fix the
  prose, don't leave a stale description standing — e.g. the 2026-07-15
  rebaseline that moved `DESIGN-NEW-FUNCTIONALITY.md`'s old phase-by-phase
  detail to `docs/archive/` rather than let it keep drifting out of sync
  with the ADRs it duplicated). If you find a conflict while working —
