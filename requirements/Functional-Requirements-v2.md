...
# # GMAtlas — Additional Design Principles
#### NEW!
This document specifies a redesign of GMAtlas's behavior from any previous requirements. 

All previous requirements must indicate when they have been superceded by commenting out the invalid content and referencing the location of the new requirement by doc and section.

This document should specify GMAtlas's behavior completely enough to rebuild the application from scratch without access to the original source. It describes **what the system does and the exact rules that govern it**, not how the current codebase is organized. 

Any inconsistencies must be logged in /docs/design/inconsistencies_v2.md

Same for where a rule is subtle or easy to get wrong, it is called out explicitly.
...
## 1.1 Frictionless Empowerment

#### Purpose:

The GMAtlas must provide a gameplay engine that automates the following use case and gives the GM/user the freedom to select and reshape the story as desired:

> "At the start of a scene, the GM will inform you where your character is, 
what’s going on nearby, and anything else useful, important, or obvious 
that you should know. They might ask you a couple of questions about 
where you are or what you’re doing. 
There’ll often be a reason behind this scene, driven by what happened 
in the scenes before it: perhaps you came here because of a clue left by 
raiders, or because you’re looking for a specific person. This is setting the 
scene. Once the GM has finished setting the scene, you and your fellow 
players can ask questions about the situation and choose for your 
characters to act within the scene: move around, talk to other people, 
or otherwise do things. Once you’ve reached a point where you can’t do 
anything further towards your goal, or you’ve gained a new goal that 
requires you to go somewhere else, the scene ends, and a new one begins.
>
A scene is defined by its location, the events occurring there at a specific 
time, and the people who are there. These elements are all facts about the 
scene that can influence the actions that characters wish to and are able to attempt.
>
Also, Scene Beats, or "Truths"  (however described in the rulesets being used): 
As Truths represent significant facts, they represent the things that are 
important to know about the scene, the kinds of elements which are most 
obvious when a scene begins. Imagine the details that are most obvious 
when a scene begins in a movie or TV show, or the elements the author 
describes as scenes unfold in a novel. There may be lots of insignificant 
facts, but they’re just that — insignificant — so they don’t need to be defined. If something becomes important, it becomes a Truth. If a Truth is no longer important, 
or stops being true, it stops being a Truth."


---


> The GM is focused on the current moment, appreciates timely suggestions, and only reaches for reference material when needed.

The app exists to let someone **run a four-hour session without thinking about the software.** Every feature is judged against that. If a control makes the GM stop and hunt, it has failed, no matter how powerful it is.
...
The intended gameplay:
	...
	• Rich setting with persistent factions
	• Low combat but high tension
	• Meaningful consequences
	• Strong procedural support
	• Minimal journaling
	• Very low cognitive overhead
	• Mechanics that reward exploration, investigation, logistics, diplomacy and discovery more than killing enemies
Because no published RPG optimizes all of those simultaneously, the best solution is a Rules Lens Architecture, where each game contributes only what it does best.
## 1.2 Design Principles

Users must be able to read details about NPCs and situations and make decisions associated with those that roll dice and maintain story continuity. This could mean mapping Moves the character(s) may take to interact with the NPCs or respond to the situation. Net to each Entity or 5-W description should be a small superscript lightbulb icon that loads relevant Moves to the Advisor.

Automated suggestions can deviate from Story continuity for anything but primary mission objective, such as priorities, challenges, locations, secondary mission opportunities, relationship status, etc.

The 5-Ws each can include oracle suggestions (a lightbulb that loads the suggestions into the Navigator) but take user input that overrides current content or oracle results pasted to the 5-W description textboxes. 

When manual edits conflict with other story beats and situational elements, alert the user of conflicts and offer Oracles as options to repopulate with consistent response.

Identify entity attributes that dont fit the scene and suggest oracles or themes to manually align to the story.

Check all edits and prepopulated content and flag when an NPC or Entity conflicts with the scene. Display the notice with a red triangle warning icon in a status log box on the Advisor panel

When clicking an empty or initially populated field in the Composer, suggest oracles in the Advisor fitting the NPCs, location, and open trackers to keep event cohesion

## 1.3 Situation Model & Story Shifts
### Consequences and Events
* When something happens, there might be cascading or chained or ripple effects that follow the player. This could be 
	* presented in the Advisor as a list of options to include in the Narrative Composer as a part of the scene:
		* list of opportunities (e.g., quests)
		* list of consequences such as financial expenses (e.g., fines), or physical danger, 
		* list of hazards to the immediate area that the character must detour to address

* This should be displayed in the breadcrumbs of events displayed below the logo at the top left nav. Clicking the breadcrumb should open that scene and highlight the description field.

- **Goals**: There will always be a obvious goal. Assess the scene and ask the user to confirm or edit the goal. Secondary goals can exist that are needed to accomplish to main goal. All other dynamic options must filter and be relevant against this goal. Ex: a random encounter of a forest fire would not be relevant on an airless moon. Hazards can distract or obstruct from the primary goal and create side quests or additional objectives in the encounter, but should not invalidate the primary goal.
...
GMAtlas is a **campaign operating system** for solo and GM-run sci-fi tabletop RPG play: a static, local-first, installable-as-PWA web app with **no
backend** — a single HTML document, a bundled JS file, and browser storage. 
It helps a GM run a session by tracking one evolving campaign state (a "scene" part of an "act" within an overall "campaign"), generating oracle-driven narrative content on demand, and
...
4. **Context is the foundation of every recommendation.** All entity selections and description textboxes establish the scene context. Based upon relationships between entities and content within descriptions of the active 5-W situation model plus the wider campaign graph — not from a canned script.
5. **The primary workspace** Three panels/drawers, not a maze of screens, can be side-by-side on a browser workspace of a workstation or tablet, but just one panel/drawer at a time on a phone.
6. **Relationships create meaning.** Entities are meaningfully connected via typed, weighted relationships and free-text @mentions, not siloed records.
...
8. **Campaign data is sacred.** Nothing the GM has entered is ever silently
   ...
   facts, foreshadowing history) are append-only by design — "flag, don't delete."
...
### 1.3<mark style="background: #FFB8EBA6;"> Visual tiers (UX contract, not a data model)</mark>
...
This schema is brought forward. Adjust the following as necessary to fit the newly mentioned requirements throughout this document.
...
#### NOTE
Alert user when data is not formatted for the current schema or design and there are issues with conversion.
...
#### NEW!
A new scene can be prepopulated with values derived from the last scene but extended using named shifts and situation dials to move the story forward in a logical way. 

Situation dials include (threat, mystery, resources, reputation, stress).

Pre-population of a new scene must be an optional choice where manual creation can be performed without system conflict.

Complicate and other situation modifiers must be intuitive and applicable to the circumstances.  

'advance time' advances timers and introduces opportunities, threats and consequences. Treat this as a "move to the next turn or round" during situations using initiative or just move to the next scene and process everything that happened in the meantime.
...
Design the suggestions in the Composer to be based upon tags, and tags assigned to the Oracles that would fit the suggestion for the purpose of adding new oracles in the future.

When running oracle tables based upon a tag reference, then treat all tables with that same tag as a single table with equal weight on each table row. The point of this is to allow incorporation of several sources of random tables to increase variety. 

The addition of Oracle tables and referenced tags would be managed in the Settings under Oracle Tags. The tags would not be seen in the Oracles panel.
...
- **Usage tracking:** every real roll increments a per-top-level-table-
  group usage counter. This is read later purely as a tie-breaker (§5.9), never as a primary ranking signal.
...
- <mark style="background: #FFB8EBA6;">Composition rolls </mark>(each independently, missing/empty table → `null`,
...
- **Faction pacing bump**: every scene generated (via Continue Story or a
  ...
  is only "due" as a derived boolean the Composer can nudge about.
...
#### NEW!
Cast includes #character #npc (plus either #antagonist  or #bystander)
NPC are given the #NPC tag. NPC characteristics are dynamically generated to fit the scene using location and plot points (colonists in colonies, survivalists in wilderness, spacers on starships, merchants or thugs in markets, bounty hunters or drunks in bars, etc).
...
### 5.6a Player actions / Moves

The default action/move system is Starforged. 
Combat mechanics use 5PFH.
All of the moves are available in a "moves list" popup from the Composer organized into a collapsing tree of move groups. 
Expanding to a specific move will show the details of the move in the same format typesetting as the IronSword-Starforged Reference Guide. 
Consequences that reference another Move, such as Pay The Price, will open the moves list to that move.
Each move will have a "roll" button that pops up a selection of what stat(s) are used for the roll. The stat of the selected NPC entity will be used. If no entity is selected, prompt the GM to select one.

#### Move list suggestions

When a move is completed, related moves common to occur afterward should be presented as suggested moves links in the Advisor to open up the Moves entry.

What a scene is defined and the moves list is opened, it should highlight the entries that would be common responses to the current scene.

After a dice roll, identify affected timers and meters and present the possible changes in the Composer for the GM to approve or adjust before clicking Commit to apply changes.

#### Dice roll results

Use the rulesystem to determine appropriate scoring and use of timers and meters, but allow a switch to another system to map the old timer or meter to an equivalent in the new system. Do not delete the timer or meter information in case the rulesystem is changed back.

Within moves involving dice roles (such as starforged where consequences occur relative to strong or weak hit or miss), the app should present the impact in the Composer for the GM to apply or edit conditionally per consequence to meet the narrative of the story.
When rolling dice, have the result post to the Composer for resolution and optional Commit to the Journal.

Apply the resolution to the stats, timers, meters, assets and other factors, as well as form a 1-3 sentence summary narrating the gameplay that is first displayed in the Composer for the GM to edit and Commit to the Journal.
...
- "Under pressure" = the open thread with the highest fill ratio.
  ...
  observational, surfaced (e.g. by Composer) and never auto-modified.
...
### 5.9 Composer (`advise`, Story Options, Narrative Composer)

See 6.3

#### NEW!
The scene summary should be formatted like this:
...
{consequences}
#### Location Tacker
* breadcrumb at top of Navigator Panel showing site, district, region, system, sector
* breadcrumbs and entity links open the entity editor
* [[Site]] - location of the scene
* [[Region]] - Planet 
	* Presence is determined by: 		
		- relationship: located_at Star-System 
		- Assets at location 		    
		- Homeworld 		    
		- Bases of Influence 		    
		- Governed locations 		    
		- Faction membership		    
		- Region containment tree (deep traversal)
* [[System]] - Star system 
* [[Sector]] - Star sector
...
%%
Dynamic events should factor into transactions, such as:
* random success impact - ex: a needy customer,
* random consequence impact - ex: spoilage, corrosion, equipment failure, ship system failure,
%%
...
%%
Mission is the main kind of event, but with a variable start time and duration requirement

Missions have progress clocks that reflect steps to complete or actual timers with triggering events listed for clock progression.
%%
...
%%
Upon addition of a new faction entity, include a field for size of the faction and generate the FacCreds or other attributes to match the size appropriately. 

All factions within the location (system down to site) should participate in the faction turn calculation. 

Faction events that occur should add an entry to the opportunities and consequences affecting the players. 
%%
...
- A single mount call builds the entire static page skeleton **once**:
  header, breadcrumb, the Storyboard region, the always-visible
  Composer aside, a document-viewer overlay panel, a search overlay, a
...
- **Render loop**: a single re-render function runs on every store
  ...
  the Composer and the Navigator panel from the *same* gathered
...
### 6.2 The Storyboard (primary workspace)

#### NEW!
Protagonists
    (in-scene NPCs tagged as party-facing characters, derived live),    
Antagonists NPCs relevant to the scene for interacting with the PCs; (every other in-scene NPC, derived live), suggested in the Advisor, derived from NPC Oracles, must fit the scene, regenerate until suitable for the scene; these have all the NPC attributes and those added to the Composer are saved to the Entities List.
Bystanders (a GM-curated add/remove list, since there's no reliable derived query
    for "nearby but unmentioned"), these are functional roles like shopkeeper, pedestrians, laborers, corporate employees, etc.; These have no stats unless engaged. 

Bystanders can be converted to NPCs Antagonists where stats are auto-generated based upon profiles derived from tags 

The menus open up panels designed to fit one of the three sections of the Body (for desktop and tablets). Phone UX should just have the tab menu listing the panels, but pin Composer, Story Beats as the first two menus.

#### Navigation, Panels & Drawers

The Storyboard is linked on the top nav bar when it needs to be minimized when other windows cover it, such as opening multiple drawers that expand and connect to each other and overlay the main workspace. 

##### Main Nav buttons

These other panels open in drawers from the top nav
- Storyboard - When showing the Navigator and Composer together, they will always be grouped into one panel for consistent aesthetics. This combination can be called the Storyboard. This would not be the case for phone UX as they would always be two panels that are anchored as the first two tabs in the tab menu. 

- Storyboard (open-book icon)
- Party
- Colony
- Journal
- Settings (gear icon)
...
On first open, or after closing all drawers in the tab menu, there are two workspaces:
1. "Storyboard" - the left 2/3 of the workspace is a single panel. The Storyboard panel has two sections that are one panel on a desktop computer or tablet, but separated into panels pinned as the first two tabs in the menu for phone devices.
- "Narrative Composer" ("Composer") - left side (previously called "Story Dashboard" or "Storyboard")
- Story Navigator ("Navigator") - Active scene beats - right side (previously called "Narrative Composer")


1. Tab Menu ("Sandbox") - the default panel in the right 1/3 that is used as a tab menu for the drawers that open from the right edge menu. 
- **"Advisor"**, replaces legacy reference to "Co-Pilot". This has ephemeral content such as recommendations or links to filtered list of oracles to replace content in the Composer that doesn't fit the scene. When suggestions are available for user choices such as a filtered list of oracles, these are presented in a popup panel that floats over the Navigator. On a mobile device, this would display as a smaller popup the does not obstruct the active field to allow consideration of options without moving between tabs. This is not opened for reference as it is displayed automatically and only contains references and advice supporting the current situation. This is closed when you hit ESC or by clicking to the "Close" (X) button.
##### Right Sidebar buttons

Right nav opens these menu options into drawers/panels:
Guide, Oracle, Cast, Faction Events, Trade, Docs, Gallery, Battlemap, Graph.
#### Panels (and tabs)
Drawers will open in the tab menu by default, but can be extended to the middle and left by anchoring to the left-most open drawer.
* additional Panels anchor to the left of the leftmost Panel until all three slots are filled
* Panels have right/left arrows to shift position and exchange position with the Panel to left or right.
* the right edge Panel also supports a tab group of Panels
* each opening of a Panel starts as a new tab in the menu until selected to shirt to the left
* Panels can be repositioned to any of 3 locations 
* clicking the Storyboard top nav menu will collapse all drawers back to the right panel, showing only the Composer, Navigator and the Advisor
...
- **Left column**: a current-location banner, a shared Activity picker
  (drives the Composer's Rules Lens suggestion), then five independently
  ...
  - **HOW**: a Focus field only (the Activity picker and its Rules Lens
    suggestion live in the shared header / Composer respectively, not
    duplicated here).
- **Right column** (sticky): the **Advisor** — a deliberately
  **read-only** live preview of `composeNarrativeDraft()`, recomputed
  fresh on every render, with only Oracle roll, <mark style="background: #FFB8EBA6;">Select from options that paste to the original field</mark>, Copy and "Send to Journal" actions. It
...
### 6.3 The Composer Panel (always visible)


NPCs who are not part of the #party or #character are automatically suggested in the Composer at the beginning of the scene (with option to reroll), "rolling" an NPC is derived from NPC Oracles, must fit the scene, regenerate until suitable for the scene and the role (antagonist, bystanders). 
Additional NPCs can be generated on demand. 
If a newly generated NPC is not favorable, the user can select specific attributes manually, and the manual selections can define other criteria (which are locked if the NPC is rerolled again after one or more fields are manually selected). If the field content is cleared and then the NPC rerolled, that field is treated as new and gets updated with the random information.
The thumbnails have +/- buttons to move them to the Storyboard or delete.

<mark style="background: #FFB8EBA6;">This panel provides the entity details for consideration before being added to the Composer.</mark>
* Small (48px) thumbnails (of )circle icons) of NPCs with name underneath
* Use a Generic light gray silhouette on white background for NPCs with no picture.
...
Replace this design as follows:
* All content are in Drawers
* The default workspace has three drawers open by default: "Narrative Composer" (left), Story Navigator (middle) and Advisor (right)
* * The Storyboard is a Drawer that is persisted in the Left edge, and remain as a primary workspace when all drawers are collapsed. It can be moved, but never closed unless replaced by another drawer to ensure population of all three drawer positions.
* The Narrative Composer contains the where/who/what/why/How (in that order)
* The Story Navigator is its own Drawer and remains in the middle section attached to the Storyboard when all drawers are collapsed.
* These two drawers can be moved to other drawer positions as long as three drawers are open at one time.
* The Advisor defaults to the right drawer position.
* All drawers have left/right triangle-arrow icons to shift position on the workspace as well as a close button.
* Other drawers initially open into a tab menu on the leftmost drawer. If moved to the left, the drawer it replaces moves back to the right one position until being added to the tab menu.

* Top navigation under the title includes a breadcrumb timeline of the last entity modified. This continues to push the list of breadcrumbs to the right as the new one appends to the left.
* If indicated in design, a drawer can be the width of two or three drawers. Three drawers just becomes a full window (3x) drawer that can be closed or added to the menu.
* If the drawer is 2x, the tab menu is displayed in the rightmost drawer (a single drawer or the 2x drawer, as applicable)
* A 3x drawer can pop out and function as a separate browser window that is still tied to the main workspace for any content changes or click the arrow to return to the app.
* On mobile phone devices, All drawers are only single-width and are managed through the tab menu.
* Treat tablet mobile devices as regular layout.
