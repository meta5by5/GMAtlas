...
- [x] Phase 15 — Rules Profiles + multi-campaign (`design/adr/rules-profiles-multi-campaign.md`): multiple independent campaigns, each assigned a shared, reusable Rules Profile bundling ruleset choices, which modules (Colony/World Tracker/Trade/Battlemap/Graph/Faction Events) are visible, and what fills the Storyboard's three positions; migration seeds a Default profile plus a 5PFH profile; new Settings → Profiles & Campaigns tab
