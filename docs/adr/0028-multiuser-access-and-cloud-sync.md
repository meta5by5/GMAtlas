...
Recording that plainly is what this ADR is for — matching this repo's
own convention (`CLAUDE.md`: "when a design choice is ambiguous... write
a short ADR" for "a persistence-behavior change") of writing down a real
fork explicitly instead of discovering it mid-implementation. The
rejected alternative this ruled out is recorded in
`docs/archive/adr/0028-multiuser-access-and-cloud-sync.md`.
...
See `docs/archive/adr/0028-multiuser-access-and-cloud-sync.md` (slotting
this in as the next committed phase, and silently folding it into the
existing Sync Adapter backlog bullet, were each considered and
rejected).
