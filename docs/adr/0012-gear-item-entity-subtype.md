...
See `docs/archive/adr/0012-gear-item-entity-subtype.md` (modeling gear as
a statblock kind on the existing `asset` type, one universal gear template
shared across systems, pick-one-template like Bestiary, and folding
"wetware" gear into `domain/cybernetics.js`'s Strain system were each
considered — the last deferred rather than adopted, the rest rejected).
