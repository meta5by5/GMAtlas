> **Archived 2026-07-15.** This is the verbatim project `CLAUDE.md` as of
> that date, frozen before a rebaseline pass (`docs/adr/0040-story-
> dashboard.md`) that fixed stale facts (the phase-status line, the
> directory map, the Reference Library's LFS→GitHub-Release change) and
> tightened prose. Every rule, architectural constraint, and "Known
> non-issue" here should still be present in the live `CLAUDE.md` — this
> archive exists for wording history, not because anything load-bearing
> was dropped. If this archive and the live `CLAUDE.md` disagree on an
> actual rule, the live file wins.
