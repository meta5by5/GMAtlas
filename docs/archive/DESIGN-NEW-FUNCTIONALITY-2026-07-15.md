> **Archived 2026-07-15.** This is the verbatim `DESIGN-NEW-FUNCTIONALITY.md`
> as of that date (Phases 0–11 complete, Phase 10's "new features" fully
> detailed), frozen before the doc was rebaselined around Phase 12 (the
> Story Dashboard, `docs/adr/0040-story-dashboard.md`) and trimmed to stop
> duplicating `PROGRESS.md`'s Status Summary and the individual ADRs — see
> the live `DESIGN-NEW-FUNCTIONALITY.md` for current content. Kept here for
> the full phase-by-phase rationale/detail that doesn't need to stay in the
> active roadmap file. If this archive and the live doc (or an ADR)
> disagree, they win — this is a history record, not a second source of
> truth.
...
ADR 0008 also explicitly declined an Expedition four-dial tracker,
structured Diplomacy fields, a Discovery-classification field, a
Noncombat-approach taxonomy, and a mechanized session-composition budget —
each was put back to the user individually afterward (non-binding,
suggestive) rather than left as a final call. Four of five got reversed or
redirected; see `docs/adr/0009-situation-engine-revisited.md` and the new
Phase 10 items below. Only the session-composition budget stayed declined.
...
- **Expedition trackers — new `domain/expeditions.js`** (`docs/adr/0009-situation-engine-revisited.md`, reversing one of ADR 0008's declines per explicit user direction: "treat those four as trackers, not lifecycle states"). A Thread tagged `kind: 'expedition'` (the same tagged-Thread convention Contracts/Faction Pressure already established) gains three 0–10 dials — `supplies`, `exposure`, `morale` — alongside its own existing clock, which already covers "progress." `copilot.js` gains a threshold rule (low Supplies, high Exposure) matching how Stress/Resources thresholds already surface a dilemma. *Effort: low-medium.*
- **Diplomacy fields on the Faction card** (`docs/adr/0009`, reversing another ADR 0008 decline) — three new fields, `fear`/`need`/`secret`, alongside the existing `hq`/`leadership`/`scenarioSeed` (`ensureFactionFields`). A possible follow-on, not committed: surfacing a faction's fear/need in the Co-Pilot when the HOW workspace's Activity is set to "Negotiate." *Effort: low.*
- **Suggestion Lenses for "What Happens Next?"** (`docs/adr/0009`, redirecting ADR 0008's Discovery Quality and Noncombat Resolution declines into one shared mechanism instead of a stored field and a resolution mechanic). Root problem this also fixes: "Continue Story" and "What Happens Next?" are wired to the exact same handler today (`ui/shell.js`) — two buttons, no behavioral difference. New: two small lens tables (a "Discovery Lens" — Technology/History/Politics/Religion/Biology/Physics/Trade/Culture — and an "Approach Lens" — Violence/Negotiation/Stealth/Science/Engineering/Economics/Social leverage/Exploration) plus a lens → Oracle-category mapping (same lookup-table shape as Phase 9's Activity → Rules Lens); *What Happens Next?* offers a few lens chips instead of immediately generating, and picking one calls a new `suggestNextWithLens()` that filters toward the chosen lens's categories before rolling — "Continue Story" keeps today's unchanged, no-input behavior. *Effort: medium* — a small new mechanism, not pure content, so it stays Phase-10-shaped despite being user-prioritized (same tension Trade & Logistics already navigated per ADR 0003).
- **Shipyard companion link** (item E's remainder) — **scoped** 2026-07-08, `docs/adr/0029-shipyard-deckplan-builder.md`, after the real GitLab source (`gitlab.com/IvanSanchez/geomorph-shipyard`) was read. Not a new drawer: a gridless Battlemap map, a Gallery-style tagged/searchable part library (the direct fix for the reference tool's own "scroll and hover, no filter" limitation), one new capability (icon rotation/flip), and a pure tonnage-summary function — no separate save/load file format, parts live in the campaign document like everything else. Not yet built.
...
- **Sync adapter / Crew Log shared database** (was item F) — the backend question is answered (Supabase, 2026-07-08 request), but see `docs/adr/0028-multiuser-access-and-cloud-sync.md`: bundled with a full multi-user/auth/membership-tier ask that's a genuine architectural fork from this app's local-first design, recorded as long-horizon and explicitly **not started**, pending its own architecture-decision pass.
- **Rules Constitution data for Traveller and Stars Without Number** — **Done** (2026-07-06, on direct user request; see `docs/adr/0010-traveller-swn-content.md`), as honestly-labeled original content since neither system has a source PDF in this repo's library. Traveller gained a `data/rulesets.js` character ruleset — STR/DEX/END/INT/EDU/SOC collapsed to this app's usual small-rollable-modifier abstraction (not the literal 2-12 UPP + derived-DM system), finally giving `domain/dice.js`'s `rollTraveller` (2d6 vs 8, unused since Phase 6) an actual character sheet. This reverses one specific call from `docs/adr/0002` (which had rejected a Traveller character template) — the reversal and its reasoning are recorded in ADR 0010, not silently overwritten. SWN gained a new "Stars Without Number" oracle group (Faction Action, World Tag oracle tables) rather than a character template, since SWN's named areas (Sector/World generation, Factions) were never character-sheet-shaped in the first place — Faction Action is reachable as a second 🎲 button on the Faction Pressure Track's Faction card, alongside the existing Hostile-flavored Faction Activity roll. `rulesConstitution.js`'s status strings for both updated accordingly. Sector generation (SWN) remains future work.
- **"Services to automate"** (`requirements/initial design inputs/gameplay-goals.md`): faction turns and rumors — **Done**, scoped deliberately small rather than the full Living World Engine (pack 38)/Scenario Engine (pack 36) those packs describe: `domain/factions.js`'s `advanceFactionTurns()` is a GM-triggered bulk action (a "🎲 Advance Faction Turns" button in the Journal drawer), not a background scheduler or live simulation — every faction with a Pressure Track advances by one tick and rolls a Faction Activity rumor, journaled together as one turn. Reminders/timers still overlap with Phase 6's Narrative Trackers item and aren't new scope here.
- **SWN content deepened + CWN cybernetics borrowed** — **Done** (2026-07-06, on direct user request, after the real SWN Revised Deluxe / CWN Deluxe PDFs were added to `assets/docs/`; see `docs/adr/0011-swn-cwn-content.md`, which extends `docs/adr/0010` above rather than superseding it). Reading the actual books informed which well-documented CONCEPTS to reimplement — still original phrasing/data throughout, not a transcription. Faction creation gained real depth: `force`/`cunning`/`wealth` stats (0-10) plus a growing Assets list per faction, and a stat-driven turn-resolution mini-game (`resolveFactionTurn` — d10 + the acting stat vs. a flat difficulty, wired into both the existing bulk faction-turn action and a new per-faction "▶ Turn" button). NPC deepening: `deepenNpc` rolls new Stereotype/Want/Complication tables onto an EXISTING NPC's Overview, distinct from `generateNpc`'s from-scratch chain. Bestiary/place/adventure-seed styling: three small combinatorial oracle groups (Xenobestiary, Site Concept, Adventure Seed) plus a `domain/worldbuilding.js` generator each, in the same "building-block" style SWN's own creature-creation chapter is known for. CWN's best-known subsystem — cyberware costing Strain against a limited capacity — landed as a new `domain/cybernetics.js` module and a Cybernetics section in the NPC inspector; deliberately NOT a new `RULES_PROVIDERS` entry, since CWN was never one of the six systems the original Rules Constitution roster named (see the ADR's Alternatives Considered).
